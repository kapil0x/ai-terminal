...
exports.ArchitectureExplorer = void 0;
const vscode = __importStar(require("vscode"));
class ArchitectureExplorer {
    constructor(backgroundAnalyzer) {
    ...
    async showArchitectureOverview() {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('aiTerminalArchitecture', 'Project Architecture Overview', vscode.ViewColumn.One, {
                enableScripts: true,
    ...
    generateArchitectureHTML(patterns) {
    ...
    generateArchitectureSummary(patterns) {
        ...
        const mostCommonPattern = patterns.reduce((prev, current) => (prev.confidence > current.confidence) ? prev : current);
        return `
            <h3>📊 Architecture Summary</h3>
            <ul>
    ...
    inferArchitecturalStyle(patterns) {
    ...
    async analyzeSpecificPattern(pattern) {
        try {
            // For now, show a simple information message
            // In the future, this could trigger a deeper analysis
            vscode.window.showInformationMessage(`Deep analysis of ${pattern} pattern is coming soon! This will show:
                • Files implementing this pattern
                • Pattern usage statistics
                • Improvement suggestions
                • Related patterns in your codebase`, 'OK');
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to analyze pattern: ${error}`);
        }
    }
    async showFilesWithPattern(pattern) {
        try {
            // This would ideally show files that implement the specific pattern
            // For now, we'll show a placeholder
            vscode.window.showInformationMessage(`Files using ${pattern} pattern will be listed here in a future update. This feature will show:
                • Specific files implementing the pattern
                • Usage locations with line numbers
                • Pattern adherence score
                • Refactoring suggestions`, 'Got it');
        }
        catch (error) {
...
exports.ArchitectureExplorer = ArchitectureExplorer;
